...
export default function HomePage({ products, lang }) { // lang prop qo'shildi
  ...
  return (
    <div className="bg-white min-h-screen font-sans selection:bg-blue-600 selection:text-white">
      <SEO title="RuAuto TRAILER | Продажа полуприцепов в России" />
        ...
        <div className="max-w-[1500px] mx-auto w-full px-6 md:px-10 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <h1 className="text-4xl md:text-7xl font-black leading-[0.95] tracking-tighter text-slate-900 mb-8 uppercase">
                НАДЕЖНАЯ <br />
                ПРИЦЕПНАЯ <br />
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  <img
                    ...
                    alt="Semi-trailer"
                    className="w-full h-full object-contain drop-shadow-[0_40px_70px_rgba(0,0,0,0.12)] relative z-10"
                  />

                  {/* Model tag - rasm bilan birga almashadi */}
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="absolute bottom-6 right-0 bg-white/90 backdrop-blur-md border border-slate-100 px-6 py-3 rounded-2xl shadow-lg hidden md:block"
                  >
                    <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest block mb-1">Модель</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-7">
          {sortedProducts.map((product, idx) => (
            ...
            >
              ...
              <Link to={`/product/${product.id}`} className="relative aspect-[4/3] p-8 overflow-hidden bg-slate-50/50 rounded-t-3xl flex items-center justify-center">
                <img
                  src={product.image}
                  alt={product.name}
      ...
      <section className="py-12 px-6">
        {/* Padding md:p-24 dan md:p-16 ga kamaytirildi, burchaklar radius [2.5rem] qilindi */}
        <div className="max-w-[1500px] mx-auto bg-white rounded-[2.5rem] p-8 md:p-16 relative overflow-hidden text-center border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.04)]">
          ...
          {/* Dekorativ orqa fon elementlari (Yana ham nozikroq) */}
          <div className="absolute top-0 right-0 w-1/4 h-full bg-gradient-to-l from-blue-50/30 to-transparent pointer-events-none" />
          <div className="absolute bottom-0 left-0 w-1/4 h-full bg-gradient-to-r from-blue-50/30 to-transparent pointer-events-none" />

          <div className="relative z-10">
            {/* Badge (Ixchamroq - mb-10 dan mb-6 ga tushdi) */}
            <div className="inline-flex items-center gap-2 bg-blue-50 px-4 py-1.5 rounded-full mb-6 border border-blue-100/50">
              <MessageSquare size={14} className="text-blue-600" />
              <span className="text-[9px] font-black text-blue-700 uppercase tracking-widest">Консультация</span>
            </div>

            {/* Sarlavha (text-7xl dan text-5xl ga tushirildi) */}
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-8 tracking-tighter leading-tight uppercase max-w-3xl mx-auto">
              Нужна помощь <br />
              <span className="text-blue-600 font-black">в выборе техники?</span>
            </h2>

            <div className="flex flex-col md:flex-row items-center justify-center gap-6">
              {/* Tugma (py-6 dan py-4.5 gacha ixchamlashtirildi) */}
              <Link
                to="/contacts"
                className="group relative inline-flex items-center gap-3 bg-slate-900 text-white px-10 py-4.5 rounded-xl font-black text-[11px] tracking-widest hover:bg-blue-600 transition-all duration-300 shadow-xl shadow-slate-200 active:scale-95 uppercase"
              >
                Связаться с нами
                <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
              </Link>

              {/* Info qismi (Chegara va paddinglar ixchamroq) */}
              <div className="hidden md:flex items-center gap-3 text-left border-l border-slate-200 pl-6">
                <div className="w-9 h-9 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center shrink-0">
                  <ShieldCheck size={18} />
                </div>
                <div>
                  <p className="text-slate-900 font-black text-[10px] uppercase tracking-wider">Бесплатно</p>
                  <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest leading-none">Ответ за 15 минут</p>
                </div>
              </div>
            </div>
          </div>
